...
| `ELEVENLABS_MODEL_ID` | `eleven_turbo_v2_5` | Model |

**Free-tier behaviour.** The free plan is a monthly character quota. When it
runs out ElevenLabs answers 429; the endpoint logs the status, returns 503, and
the assistant speaks with the device voice. Nothing breaks and the citizen sees
no error. Setting `ENABLE_ELEVENLABS_TTS=false` skips the call entirely.
...
- a **Firebase ID token**, forwarded by the agent, proves which citizen is
  speaking. The uid comes from the verified token, never from the body — an
  agent cannot file in someone else's name;
...

{
  ...
  "idToken": "<the citizen's Firebase ID token>",
...
```jsonc
{ "action": "submit_report", "idToken": "...", "draft": { ... }, "confirmed": true }
```
...
4. Declare `idToken` as a **dynamic variable** the client passes when the
   conversation starts, and have both tools forward it.
